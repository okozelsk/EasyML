...
namespace EasyMLCore.MathTools
{
    ...
    /// </summary>
    [Serializable]
    public class WeightedAvg : SerializableObject
    {
        ...
        //Static methods
        public static double[] AverageVectors(List<double[]> sourceVectors, List<double> weights)
        {
            double[] output = new double[sourceVectors[0].Length];
            for (int i = 0; i < output.Length; i++)
            {
                double wSum = 0d;
                double wMem = 0d;
                for (int sIdx = 0; sIdx < sourceVectors.Count; sIdx++)
                {
                    wMem += sourceVectors[sIdx][i] * weights[sIdx];
                    wSum += weights[sIdx];
                }
                output[i] = wMem / wSum;
            }
