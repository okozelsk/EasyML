...
|[RVFLModel](./EasyMLCore/MLP/Model/RVFL/RVFLModel.cs)|Implements the deep RVFL (Random Vector Functional Link) model, which consists of the RVFL preprocessor and any MLP end-model. Like MLP, the preprocessor consists of one or more fully connected hidden layers. The weights on the hidden layers are random and fixed. The EasyML implementation of the RVFL deep model solves the problem of signal saturation and vanishing during transitions to deeper hidden layers. It is only necessary to enter the scale factor for the weights on the first hidden layer, whose weights are from a uniform distribution. Further layers then apply random weights from a gaussian distribution depending on specific activation functions to ensure optimal signal passage. Each hidden layer can consist of several pools of neurons with different activations, where for each pool it can be specified whether its activations will be used as predictors for the MLP end-model. In addition to the predictors from the pools, the original input vector can also be passed to the MLP end-model. The MLP end-model should be just a simple network without hidden layers or another MLP model composed of such simple networks. Build method requires instance of the [RVFLModelConfig](./EasyMLCore/MLP/Model/RVFL/RVFLModelConfig.cs) class.|
