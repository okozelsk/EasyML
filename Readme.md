...
|[CrossValModel](./EasyMLCore/MLP/Model/Network/NetworkModel.cs)|Implements the cross-validated model. It has dedicated [CrossValModelBulder](./EasyMLCore/MLP/Model/CrossVal/CrossValModelBuilder.cs), which uses NetworkModelBuilder to build N NetworkModel(s), where each is trained on different training and validation datasets. In addition to the network configuration, the "fold ratio" is specified, which determines what part of the available training data will be used for the "validation fold". In other words, before starting the network training, the available training data is divided into N folds. For each network, one different fold is designated for validation and the rest for training. The model works by letting all networks perform the calculation and the final result is a weighted average of all networks weighted according to the confidence metrics achieved by the networks in the build process (bagging).|
