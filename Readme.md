...
|[StackingModel](./EasyMLCore/MLP/Model/Stacking/StackingModel.cs)|It implements a stacking model, which is a stack of several trained NetworkModel(s), the results of which are combined by the meta-learner. Meta-learner can be any MLP model. It has dedicated [StackingModelBulder](./EasyMLCore/MLP/Model/Stacking/StackingModelBuilder.cs), which uses NetworkModelBuilder to build stack members, from the outputs prepares training data for meta-learner and then uses the appropriate xxxBuilder to build the meta-learner.|
